...
const calculateFreeSlots = (preferedTime, appointments, bookDate, currentDate) => {
    const { startTime, endTime, duration } = preferedTime;
    const freeSlots = [];
    const bookingDateStart = new Date(bookDate);
    bookingDateStart.setHours(startTime.getHours(), startTime.getMinutes(), 0, 0);
    ...
    while (currentStartTime.getTime() + duration * 60000 <=
        bookingDateEnd.getTime()) {
        const slotEndTime = new Date(currentStartTime.getTime() + duration * 60000);
...
exports.default = calculateFreeSlots;
